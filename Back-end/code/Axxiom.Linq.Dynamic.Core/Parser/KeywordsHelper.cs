...
namespace Axxiom.Linq.Dynamic.Core.Parser
{
    internal class KeywordsHelper : IKeywordsHelper
    {
        ...
        public KeywordsHelper(ParsingConfig config)
        {
            ...
            //Daniel
            //if (config.CustomTypeProvider != null && !config.TryResolveTypeFromCustomTypeProvider)
            if (config.CustomTypeProvider != null)
            {
                foreach (Type type in config.CustomTypeProvider.GetCustomTypes())
                {
                    var prefix = config.CustomTypeProvider.TypePrefixIdentifier;
                    _keywords[prefix + type.FullName] = type;
                    _keywords[prefix + type.Name] = type;
                }
            }
        }
