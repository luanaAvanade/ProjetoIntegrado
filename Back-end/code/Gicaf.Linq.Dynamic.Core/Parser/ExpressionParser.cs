...
using Gicaf.Linq.Dynamic.Core.Exceptions;
using Gicaf.Linq.Dynamic.Core.Parser.SupportedMethods;
using Gicaf.Linq.Dynamic.Core.Parser.SupportedOperands;
using Gicaf.Linq.Dynamic.Core.Tokenizer;
...
#pragma warning restore 0219
        ...
        Expression ParseAggregate(Expression instance, Type elementType, string methodName, int errorPos, bool isQueryable)
        {
            ...
            if(methodName == "SelectToCollection")
            {
                callType = typeof(EnumerableExtensions);
            }
            ...
            else if (new[] { "Min", "Max", "Select", "OrderBy", "OrderByDescending", "ThenBy", "ThenByDescending", "GroupBy", "SelectToCollection" }.Contains(methodName))
            {
                if (args.Length == 2)
                {
    ...
    {
        public static ICollection<TResult> SelectToCollection<TSource, TResult>(this IEnumerable<TSource> source, Func<TSource, TResult> selector)
        {
            var list = source.Select(selector);
            var collection = new Collection<TResult>();
            foreach (TResult i in list)
                collection.Add(i);
