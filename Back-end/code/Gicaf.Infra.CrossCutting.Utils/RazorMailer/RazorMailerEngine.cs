...
namespace RazorMailer.Core
{
    ...
    public class RazorMailerEngine : IDisposable//RazorEngineHost, IDisposable
    {
        ...
        public virtual async Task LogMailError(MailMessage message,Exception e)//todo[iuri] apagar apos corrigir erros de email no ambiente cemig
        {
            ...
            {
                sw.WriteLine($"Erro ao Enviar email {DateTime.Now.ToShortDateString()} {DateTime.Now.ToLongTimeString()}" );
                sw.WriteLine($" para {message.To} assunto: {message.Subject}"  );
                sw.WriteLine($" erro: {e.Message} inner: {ObterInnerMensage(e)}"  );
                sw.WriteLine($" host: {Newtonsoft.Json.JsonConvert.SerializeObject(_dispatcher.GetConfig()) }"  );
                sw.WriteLine($"----------------------------------------------------------------------"  );
            }
        }
