...
export default function DadosDocumentacao({
	...
	const buscaPorFuncionalidade = () => {
		...
		TipoDocumentoService.findByFuncionalidade(FUNCIONALIDADE.Cadastro_Financeiro_Pessoa_Juridica)
			.then(response => {
	...
	const preencheListaDocumentacao = tipoDocumentacaoList => {
		...
		tipoDocumentacaoList.forEach(tipoDocumento => {
			if (
				TIPO_DOCUMENTO.Balanco_Patrimonial === tipoDocumento.Nome ||
				TIPO_DOCUMENTO.Demonstracao_Resultado_Exercicio === tipoDocumento.Nome
			) {
				let ano = new Date().getUTCFullYear();
				for (let i = 0; i < count; i++) {
					const newDocumento = {
						tipoId: tipoDocumento.Id,
						tipo: `${tipoDocumento.Nome} - ${--ano}`,
						files: []
					};
