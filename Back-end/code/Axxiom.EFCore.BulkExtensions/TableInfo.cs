...
namespace EFCore.BulkExtensions
{
    public class TableInfo
    {
        ...
        public static TableInfo CreateInstance<T>(DbContext context, IList<T> entities, OperationType operationType, BulkConfig bulkConfig, Type type = null)
        {
            ...
            tableInfo.LoadData<T>(context, entities, isDeleteOperation, type);
            return tableInfo;
        }

        #region Main
        public void LoadData<T>(DbContext context, IList<T> entities, bool loadOnlyPKColumn, Type objectType = null)
        {
            LoadOnlyPKColumn = loadOnlyPKColumn;
            var type = objectType ?? typeof(T);
