...
export default function ResultadoFinal({
	...
	const [
		...
		setIndicadoresForaRefMercadoRef
	] = useState([]);
	...
	const gerarMaxMin = async list => {
		...
		for (let index = 0; index < gruposLista.length; index++) {
			if (gruposLista[index].array && gruposLista[index].array.length > 0) {
				...
				montarIndicadoresForaRef(listMaxMinResponse.data.CalculoRisco_aggregate);
	...
	const montarIndicadoresForaRef = listMaxMinResponse => {
		const maxMinLastYear = listMaxMinResponse[0].Value;

		const listaFiltrada = indicadoresForaRefMercado.filter(a => {
		...
		setIndicadoresForaRefMercadoRef(listaFiltrada);
	};
