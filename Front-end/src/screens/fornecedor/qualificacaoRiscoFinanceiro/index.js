...
export default function QualificacaoaRiscoFinanceiro({key, empresa, tab }) {
	...
	const buscaQualificacao = async GrupoCategoriaId => {
		...
		const listaFiltradaMundo = indicadoresForaRefMercado.filter(a => {
		...
		setIndicadoresForaRefMercadoRef(listaFiltradaMundo);
								...
								<Fragment>
									...
									<DisplayDiv visible={tab === 3 && tabFilha === 2 && qualificacaoLista.length > 0}>
										<ResultadoAnalise
											...
											indicadoresForaRefMercadoRef={indicadoresForaRefMercadoRef}
											maxMin={maxMin}
											inserirIcone={inserirIcone}
										/>
									</DisplayDiv>
